...
import {
  Reservation, StatutReservation, TypeReservation,
...
export class ReservationsService {
  ...
  private calcMontant(reservation: Reservation): number {
    if (reservation.type === TypeReservation.LOCATION) {
      if (!reservation.dateDebut || !reservation.dateFin) return reservation.prix;
  ...
  async create(dto: CreateReservationDto, user?: User): Promise<Reservation> {
    ...
    if (dto.type === TypeReservation.LOCATION) {
      if (!dto.dateDebut || !dto.dateFin)
        throw new BadRequestException('dateDebut et dateFin sont requis pour une location');
      if (new Date(dto.dateFin) <= new Date(dto.dateDebut))
        throw new BadRequestException('La date de fin doit être après la date de début');
    }
    if (dto.type === TypeReservation.ACHAT && !dto.adresse) {
      throw new BadRequestException('L\'adresse de livraison est requise pour un achat');
    }

    const prix = dto.type === TypeReservation.LOCATION
      ? vehicule.prixLocation
      : vehicule.prixAchat;

    // Passer le véhicule en "limité" si c'est un achat
    if (dto.type === TypeReservation.ACHAT) {
      await this.vehiculesService.update(vehicule.id, { statut: StatutVehicule.LIMITE });
    }
  ...
  async findAll(statut?: StatutReservation, type?: TypeReservation): Promise<Reservation[]> {
  ...
  async updateStatut(id: number, dto: UpdateStatutDto): Promise<Reservation> {
    ...
    if (dto.statut === StatutReservation.ANNULEE) {
      await this.vehiculesService.update(r.vehicule.id, { statut: StatutVehicule.DISPONIBLE });
    }
    // Si terminée (location) → libérer le véhicule
    if (dto.statut === StatutReservation.TERMINEE && r.type === TypeReservation.LOCATION) {
      await this.vehiculesService.update(r.vehicule.id, { statut: StatutVehicule.DISPONIBLE });
    }
    // Si terminée (achat) → marquer indisponible
    if (dto.statut === StatutReservation.TERMINEE && r.type === TypeReservation.ACHAT) {
      await this.vehiculesService.update(r.vehicule.id, { statut: StatutVehicule.INDISPONIBLE });
    }
  ...
  async validerPaiement(id: number, dto: ValiderPaiementDto): Promise<Reservation> {
    const r = await this.findOne(id);
  ...
  async getStats() {
    ...
    const enAttente = await this.repo.count({ where: { statut: StatutReservation.EN_ATTENTE } });
    const confirmees = await this.repo.count({ where: { statut: StatutReservation.CONFIRMEE } });
    const enCours = await this.repo.count({ where: { statut: StatutReservation.EN_COURS } });
    const terminees = await this.repo.count({ where: { statut: StatutReservation.TERMINEE } });
    const annulees = await this.repo.count({ where: { statut: StatutReservation.ANNULEE } });
    const locations = await this.repo.count({ where: { type: TypeReservation.LOCATION } });
    const achats = await this.repo.count({ where: { type: TypeReservation.ACHAT } });

    return { total, enAttente, confirmees, enCours, terminees, annulees, locations, achats };
  }
}
